...
from get_image_feature import UNI_features
import pickle
from sklearn.decomposition import PCA
from sklearn.preprocessing import StandardScaler
import scanpy as sc
from PIL import Image
from BC3get_methods import *
...
def generation_coord_10x(
    ...
    fill_coor_df = pd.concat([coor_df, coor_df_1, coor_df_2,
                              coor_df_3])  # , coor_df_2, coor_df_3 , coor_df_4, coor_df_5, coor_df_6, coor_df_7
    # fill_coor_df = fill_coor_df.drop_duplicates(subset=fill_coor_df.columns)  # 删除重复行

    fill_spatial = pd.concat([fill_spatial_ori, fill_spatial_1, fill_spatial_2,
                              fill_spatial_3])  # , fill_spatial_2, fill_spatial_3, fill_spatial_4, fill_spatial_5, fill_spatial_6, fill_spatial_7
...
def generation_feature_10x(image_path, ori_spatial, fill_spatial):
    # sample_features_16 = UNI_features(img_path=image_path, spatial=ori_spatial, patch_size=16)
    # sample_features_48 = UNI_features(img_path=image_path, spatial=ori_spatial, patch_size=48)
    sample_features_112 = UNI_features(img_path=image_path, spatial=ori_spatial, patch_size=112)
    # sample_features_224 = UNI_features(img_path=image_path, spatial=ori_spatial, patch_size=224)
    # fill_features_16 = UNI_features(img_path=image_path, spatial=fill_spatial, patch_size=16)
    # fill_features_48 = UNI_features(img_path=image_path, spatial=fill_spatial, patch_size=48)
    fill_features_112 = UNI_features(img_path=image_path, spatial=fill_spatial, patch_size=112)
    # fill_features_224 = UNI_features(img_path=image_path, spatial=fill_spatial, patch_size=224)

    sample_features = {"sample_features_112": sample_features_112}
    fill_features = {"fill_features_112": fill_features_112}
...
def get_sectionData(section_id):
    section_list = ["151507", "151508", "151509", "151510", "151669", "151670", "151671", "151672", "151673", "151674",
                    "151675", "151676"]
    section_list2 = ['A1', 'B1', 'C1', "D1", "E1", "F1", "G1", "H1"]
    ...
    if section_id == "IDC":
        input_dir = 'D:\dataset\IDC_Data'
        adata = sc.read_visium(path=input_dir,
                               count_file='V1_Human_Invasive_Ductal_Carcinoma_filtered_feature_bc_matrix.h5')
        adata.var_names_make_unique()
        image_path = input_dir + "/image.tif"

    if section_id in section_list2:
        gene_set = list(np.load('D:\dataset\Her2st\data/her_hvg_cut_1000.npy', allow_pickle=True))
        meta_data = get_meta(section_id, gene_list=None)
        exps = meta_data[gene_set].values
        adata = sc.AnnData(exps)
        spatial = np.floor(meta_data[['pixel_x', 'pixel_y']].values).astype(int)
        position = meta_data[['x', 'y']].values
        lbl = meta_data['label'].values
        adata.uns['spatial'] = spatial
        adata.obsm['spatial'] = spatial
        adata.var['gene_ids'] = gene_set
        adata.obs['array_col'] = position[:, 0]
        adata.obs['array_row'] = position[:, 1]
        adata.obs['label'] = lbl
        adata.var.index = gene_set

        image_path = get_img('B1')
...
def get_trainData(adata, image_path, experiment, data_type, path1, coord_sf=77):
    ...
    elif experiment == 'transfer' and data_type == '10x':
