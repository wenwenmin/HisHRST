                ...
                for lr in [1e-4]:
                    ...
                    for weight_decay in [1e-4]:
                        ...
                        for cross_index in [0]:
                            ...
                            dirName = "Train_" + '_'.join(pheno_train) + "__Test_" + '_'.join(
                                pheno_test) + "__model_" + model + "__RGB" + str(
                                image_filter_percent) + "__Size_" + str(margin) + "__aug_" + aug_name + "__lr_" + str(
                                lr) + "__weightDecay_" + str(weight_decay) + "__cross_index_" + str(cross_index)
...
""")
                            ...
                            subprocess.call(
                                ['qsub', '-o', '../out/' + outDirName + '/' + dirName + '/script/out/', '-e',
                                 '../out/' + outDirName + '/' + dirName + '/script/err/',
                                 '../out/' + outDirName + '/' + dirName + '/script/run.sh'])