...
def plot_loss(outDir, loss_acc_df, name):
    ...
    plt.xticks(np.arange(0, math.ceil((loss_acc_df.shape[0] - 1) / 50) * 50 + 1,
                         math.ceil((loss_acc_df.shape[0] - 1) / 50) * 10))
...
def plot_correlation_scatter_hist(outDir, valid_labels, valid_preds, geneSymbols, scatter, name):
    ...
    for i in range(len(geneSymbols)):
        ...
        corR_df = corR_df.append(pd.Series([geneSymbols[i], corR_tmp, rmse, rmse_minmax],
                                           index=['geneSymbols', 'corR', 'RMSE', 'RMSE_MinMax']), ignore_index=True)
