...
def makeDataList(rootDir, sampleNames, clusteringMethod, extraSize, geneSymbols, quantileRGB, seed, cross_index,
                 train_equals_test, is_test, rm_cluster):
    print("### load cluster list ###")
    cluster_list = pd.DataFrame(columns=['Sample', 'Barcode', 'Cluster'])

    for sample in sampleNames:
        tmp = pd.read_csv(
            rootDir + "/" + sample + "/SpaceRanger/analysis/clustering/" + clusteringMethod + "/clusters.csv")
        tmp['Sample'] = sample
    ...
    tissue_pos = pd.DataFrame(
        columns=['Sample', 'Barcode', 'in_tissue', 'array_row', 'array_col', 'pxl_row_in_fullres', 'pxl_col_in_fullres',
                 'imageID'])
    ...
    cluster_pos_df['image_path'] = [
        rootDir + "/" + sample + "/CropImage/size_" + str(extraSize) + "/spot_images/spot_image_" + str(s_id).zfill(
            4) + ".tif" for sample, s_id in zip(cluster_pos_df['Sample'].tolist(), cluster_pos_df['imageID'].tolist())]
    ...
    for sample in sampleNames:
        tmp = pd.read_csv(rootDir + "/" + sample + "/CropImage/size_" + str(extraSize) + "/RGB_" + str(
            quantileRGB) + "/cluster_position_filter.txt", sep='\t')
...
def makeTrainDataloader(rootDir, data_list_df, geneSymbols, size, mean, std, augmentation, batch_size,
                        ClusterPredictionMode):
    data_list_df = data_list_df.reset_index(drop=True)

    print("### make dataset ###")
    if ClusterPredictionMode:
        train_dataset = SpotImageDataset(
            file_list=data_list_df.loc[data_list_df['phase'] == 'train', 'image_path'].tolist(),
            label_df=data_list_df.loc[data_list_df['phase'] == 'train', ['Cluster']] - 1,
            transform=ImageTransform(size, mean, std),
            phase='train',
            param=augmentation)

        valid_dataset = SpotImageDataset(
            file_list=data_list_df.loc[data_list_df['phase'] == 'valid', 'image_path'].tolist(),
...
def calc_cor(outputs, labels):
    ...
    for i in range(num_gene):
        corR.append(
            np.corrcoef(outputs[:, i].to('cpu').detach().numpy(), labels[:, i].to('cpu').detach().numpy())[0, 1])
...
def run_train(outDir, net, dataloaders_dict, optimizer, num_epochs, device, early_stop_max, name,
              ClusterPredictionMode):
    ### set multi GPU
    if str(device) != 'cpu':
        net.to(device)
    ...
    for epoch in range(num_epochs + 1):
        ...
        res_df = res_df.append(
            [pd.Series([train_loss, valid_loss, train_acc_or_cor, valid_acc_or_cor], index=res_df.columns)],
            ignore_index=True)
...
def makeTestDataloader(rootDir, data_list_df, model, geneSymbols, size, mean, std, augmentation, batch_size,
                       ClusterPredictionMode):
    data_list_df = data_list_df.reset_index(drop=True)

    print("### make dataset ###")
    if ClusterPredictionMode:
        test_dataset = SpotImageDataset(
            file_list=data_list_df.loc[data_list_df['phase'] == 'valid', 'image_path'].tolist(),
            label_df=data_list_df.loc[data_list_df['phase'] == 'valid', ['Cluster']] - 1,
            transform=ImageTransform(size, mean, std),
            phase='valid',
...
def run_test(outDir, data_list_df, dataloaders_dict, model, device, geneSymbols, num_features, ClusterPredictionMode,
             name):
    ...
    if ClusterPredictionMode:
        ...
        make_classification_report(outDir, valid_labels, valid_preds,
                                   ["Cluster" + str(i + 1) for i in range(num_features)], name)
...
def makeDataListSemi(rootDir, sampleNames, semiType, ImageSet, semiName):
    ...
    if semiType == "Visium":
        for sampleName in sampleNames:
            for i_imageSet in ImageSet:
                image_l = pd.read_csv(rootDir + "/" + semiType + "/ImageSet/" + sampleName + "/ImageSet_" + str(
                    i_imageSet) + "/image_list.txt", sep='\t')
        ...
        for i_imageSet in ImageSet:
            image_l = pd.read_csv(
                rootDir + "/" + semiType + "/ImageSet/ImageSet_" + str(i_imageSet) + "/image_list.txt", sep='\t')
            image_list = image_list.append(image_l, ignore_index=True)
    ...
    image_list = image_list.loc[:,
                 ['Sample', 'Barcode', 'Cluster', 'in_tissue', 'array_row', 'array_col', 'pxl_row_in_fullres',
                  'pxl_col_in_fullres', 'imageID', 'image_path', 'phase', 'semiName']]
