...
parser.add_argument('--dataDir', type=str, default='D:\dataset\DLPFC/',
                    help='Data directory (default: ')

parser.add_argument('--sampleName', type=str, default='151507',
                    help='Sample name (default: ')
...
os.makedirs(dirName + "/CropImage/size_" + str(extraSize) + "/spot_images", exist_ok=True)
### load Image
Image.MAX_IMAGE_PIXELS = 1000000000
I = Image.open(dirName + "/spatial/151507_full_image.tif")
...
for i in range(4992):
    ...
    I = cv2.putText(I, str(i), (pos_x1, pos_y1), cv2.FONT_HERSHEY_DUPLEX, 1.0, (0, 0, 0), lineType=cv2.LINE_AA,
                    thickness=2)
...
for i in range(4992):
    ...
    cv2.imwrite(dirName + "/CropImage/size_" + str(extraSize) + "/spot_images/spot_image_" + str(i).zfill(4) + ".tif",
                I2)
...
cluster_pos_df['image_path'] = [
    dirName + "/CropImage/size_" + str(extraSize) + "/spot_images/spot_image_" + str(s).zfill(4) + ".tif" for s in
    cluster_pos_df['imageID'].tolist()]
...
for i in cluster_pos_df.index:
    ...
    if sum(count_white) > pixel_th_white:
        subprocess.call(["cp", "-p",
                         dirName + "/CropImage/size_" + str(extraSize) + "/spot_images/spot_image_" + str(i).zfill(
                             4) + ".tif",
                         dirName + "/CropImage/size_" + str(extraSize) + "/RGB_" + str(quantileRGB) + "/NG/"])
        cluster_pos_df.loc[cluster_pos_df.index == i, "ImageFilter"] = "NG"
    else:
        subprocess.call(["cp", "-p",
                         dirName + "/CropImage/size_" + str(extraSize) + "/spot_images/spot_image_" + str(i).zfill(
                             4) + ".tif",
                         dirName + "/CropImage/size_" + str(extraSize) + "/RGB_" + str(quantileRGB) + "/OK/"])
        cluster_pos_df.loc[cluster_pos_df.index == i, "ImageFilter"] = "OK"

cluster_pos_df.to_csv(
    dirName + "/CropImage/size_" + str(extraSize) + "/RGB_" + str(quantileRGB) + "/cluster_position_filter.txt",
    index=False, sep='\t')
...
for i in range(4992):
    ...
    I = cv2.putText(I, str(count_inter), (pos_x1, pos_y1), cv2.FONT_HERSHEY_DUPLEX, 1.0, (0, 255, 0),
    ...
    I = cv2.putText(I, str(count_inter), (pos_x1, pos_y1), cv2.FONT_HERSHEY_DUPLEX, 1.0, (0, 0, 255),
                    lineType=cv2.LINE_AA, thickness=2)
...
for i in range(4992):
    ...
    I2 = I[pos_y1:pos_y2, pos_x1:pos_x2]
    cv2.imwrite(
        dirName + "/CropImage/size_" + str(extraSize) + "/spot_images_inter/spot_image_inter_" + str(count_inter).zfill(
            4) + "_" + str(int(center_x)) + "_" + str(int(center_y)) + ".tif", I2)

    image_list = image_list.append(
        [pd.Series([count_inter, int(center_x), int(center_y), radius], index=image_list.columns)], ignore_index=True)
...
image_list['image_path'] = [
    dirName + "/CropImage/size_" + str(extraSize) + "/spot_images_inter/spot_image_inter_" + str(no).zfill(
        4) + "_" + str(x) + "_" + str(y) + ".tif" for no, x, y in
    zip(image_list['No'], image_list['pos_x1'], image_list['pos_y1'])]
...
### mkdir
subprocess.call(
    ["mkdir", "-p", dirName + "/CropImage/size_" + str(extraSize) + "/RGB_" + str(quantileRGB) + "/NG_inter/"])
subprocess.call(
    ["mkdir", "-p", dirName + "/CropImage/size_" + str(extraSize) + "/RGB_" + str(quantileRGB) + "/OK_inter/"])
...
for i in image_list.index:
    ...
    if sum(count_white) > pixel_th_white:
        subprocess.call(["cp", "-p", image_list.loc[image_list.index == i, "image_path"].tolist()[0],
                         dirName + "/CropImage/size_" + str(extraSize) + "/RGB_" + str(quantileRGB) + "/NG_inter/"])
        image_list.loc[image_list.index == i, "ImageFilter"] = "NG"
    else:
        subprocess.call(["cp", "-p", image_list.loc[image_list.index == i, "image_path"].tolist()[0],
                         dirName + "/CropImage/size_" + str(extraSize) + "/RGB_" + str(quantileRGB) + "/OK_inter/"])
        image_list.loc[image_list.index == i, "ImageFilter"] = "OK"

image_list.to_csv(dirName + "/CropImage/size_" + str(extraSize) + "/RGB_" + str(quantileRGB) + "/image_list_inter.txt",
                  index=False, sep='\t')