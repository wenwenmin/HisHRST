...
def train(section_id, experiment, data_type, save_model_path, path1):
    ...
    if experiment == "recovery":
        (exp_data_sample, exp_data_ori, adata_sample, image_features_sample, image_features_ori, spot_coord_sample,
         spot_coord_ori, image_spatial_sample, image_spatial_ori, used_gene) = get_trainData(adata, image_path,
                                                                                             ...
                                                                                             path1)
        transformed_dataset = MyDatasetTrans(train=True, normed_data=exp_data_sample, coor_df=spot_coord_sample,
        ...
        exp_data, spot_coord_ori, spot_coord_fill, image_spatial_fill, image_features_ori, image_features_fill, used_gene = get_trainData(
            ...
            data_type, path1)
        transformed_dataset = MyDatasetTrans(train=True, normed_data=exp_data, coor_df=spot_coord_ori,
    ...
    T_max = (train_epoch - 5) * len(dataloader_loader) if True else train_epoch * len(
        dataloader_loader)
    ...
    scheduler_warmup = LinearLR(
        ...
        total_iters=5 * len(dataloader_loader)
    )
    ...
    with tqdm(range(train_epoch), total=train_epoch, desc='Epochs') as epoch:
        for j in epoch:
            ...
            for exp, pos, image in dataloader_loader:
                ...
                h, xrecon = model(image, pos)

                # + 0.1 * L1_loss(xrecon,exp)   + 0.1 * sliced_wasserstein_distance(xrecon, exp, 1000, device=device)
                recon_loss = mse_loss(xrecon, exp)
                recon_loss.backward()
                model_optim.step()
                # model_sche.step()
                if j <= 50:
    ...
    if experiment == "generation":
        ...
        adata_stage.obsm['spatial'] = image_spatial_fill.to_numpy()
        adata_stage.var.index = used_gene

        adata.write(save_model_path + '/original_data.h5ad')
    else:
        transformed_dataset = MyDatasetTrans(train=False, normed_data=exp_data_ori, coor_df=spot_coord_ori,
                                             image=image_features_ori)
        dataloader_test = DataLoader(transformed_dataset, batch_size=batch_size, shuffle=False, num_workers=0,
                                     drop_last=False)
        preds = None
        # save_model_path = f"../IDC"
        model = torch.load(save_model_path + '/recover.pth')
        all_h_features = []
        with torch.no_grad():
...
if __name__ == "__main__":
    ...
    for section_id in section_list:
        section_id = "IDC"  # mouse_brain Breast Cancer FFPE
        experiment = "generation"  # generation recovery
